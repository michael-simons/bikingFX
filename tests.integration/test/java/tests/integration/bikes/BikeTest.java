 ...
 */
package tests.integration.bikes;
...
class BikeTest {
	...
	void factoryMethodShouldWork() throws IOException {
		...
		JsonNode json = objectMapper.readTree(BikeTest.class.getResourceAsStream("/bikes/singleBike.json"));
		Bike bike = new Bike(json);
		Assertions.assertNull(bike.getDecommissionedOn());

		json = objectMapper.readTree(BikeTest.class.getResourceAsStream("/bikes/singleDecommissionedBike.json"));
		bike = new Bike(json);
